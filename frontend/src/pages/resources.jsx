...
export function ResourcesPage() {
  ...
  useEffect(() => {
    async function loadRelated() {
      ...
      setFamilySessions(extractArray(familyResponse.data));
      setSurveys(extractArray(surveyResponse.data));
    }
    loadRelated();
  }, [selectedPatient]);
  ...
  const handleFamilySubmit = async (event) => {
    ...
    try {
      ...
      setFamilySessions((prev) => [data, ...ensureArray(prev)]);
  ...
  const handleSurveySubmit = async (event) => {
    ...
    try {
      ...
      setSurveys((prev) => [data, ...ensureArray(prev)]);
      setSurveyForm({ conducted_at: "", engagement_index: 80, notes: "" });
    } catch (error) {
      console.error(error);
      alert("Nǜo foi possivel registrar a pesquisa de satisfacao.");
    }
  };
  ...
  return (
    <div className="space-y-6">
      <header className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-2xl font-semibold text-slate-800">Psicoeduca��o e apoio familiar</h2>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <div className="grid gap-6 md:grid-cols-2">
          <form className="space-y-3" onSubmit={handleFamilySubmit}>
            <h3 className="text-lg font-semibold text-slate-700">Registro de a��es psicoeducativas</h3>
            <input
              type="date"
              required
            ...
            <textarea
              className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm"
              rows={3}
              placeholder="Atividades e orienta��es"
      ...
      <section className="grid gap-6 md:grid-cols-2">
        <article className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          <h3 className="text-lg font-semibold text-slate-700">Historico de a��es</h3>
          <div className="mt-3 space-y-3 text-sm text-slate-600">
            ...
            {(!Array.isArray(familySessions) || familySessions.length === 0) && (
              <p className="text-sm text-slate-500">Sem registros psicoeducativos para este paciente.</p>
            )}
          </div>
        </article>
        <article className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <div className="mt-3 space-y-3 text-sm text-slate-600">
            ...
            {(!Array.isArray(surveys) || surveys.length === 0) && (
              <p className="text-sm text-slate-500">Nenhuma pesquisa aplicada ainda.</p>
            )}
          </div>
        </article>
      </section>
